...
load("./ProcessedData/H1models_binomial.RData")
...
m1.1_sePreds <- data.frame(m1.1_predictions,
                           mu   = m1.1preds$fit,
                           low  = m1.1preds$fit - zval * m1.1preds$se.fit,
                           high = m1.1preds$fit + zval * m1.1preds$se.fit) %>% 
...
png(file = paste0("./Figures/m1.1POD_", zval, ".png"))
m1.1POD
dev.off()
...
m1.2_sePreds <- data.frame(m1.2_predictions,
                           mu   = m1.2preds$fit,
                           low  = m1.2preds$fit - zval * m1.2preds$se.fit,
                           high = m1.2preds$fit + zval * m1.2preds$se.fit) %>% 
  left_join(mmEcoEvo, by = c("BestTaxon" = "Species")) %>% 
...
m1.2_sePreds_link <- data.frame(m1.2_predictions,
                           ...
                           low  = binomial()$linkinv((m1.2predsL$fit - zval * m1.2predsL$se.fit)),
                           high = binomial()$linkinv(m1.2predsL$fit + zval * m1.2predsL$se.fit)) %>% 
  left_join(mmEcoEvo, by = c("BestTaxon" = "Species")) %>% 
  mutate(common_name = case_when(common_name == "killer whale"~"mammal eating killer whale",
                                 TRUE~common_name)) %>% 
  mutate(depth = case_when(depth == 0~1,
...
m1.2_sePreds_link <- data.frame(m1.2_predictions,
                                ...
                                lowlink  = (m1.2predsL$fit - (zval * m1.2predsL$se.fit)),
                                highlink = (m1.2predsL$fit + (zval * m1.2predsL$se.fit)),
                                low = inv.logit(m1.2predsL$fit - (zval * m1.2predsL$se.fit)),
                                high = inv.logit(m1.2predsL$fit + (zval * m1.2predsL$se.fit))) %>% 
  left_join(mmEcoEvo, by = c("BestTaxon" = "Species")) %>% 
  mutate(common_name = case_when(common_name == "killer whale"~"mammal eating killer whale",
                                 TRUE~common_name)) %>% 
  mutate(depth = case_when(depth == 0~1,
...
m1.2POD <- ggplot(m1.2_sePreds_link, aes(x = depth, color = Broad_taxa, fill = Broad_taxa)) +
  geom_line(aes(y = mu)) +
  geom_smooth(aes(ymin = low, ymax = high, y = mu), stat = "identity") +
...
png(file = paste0("./Figures/m1.2POD_", zval, ".png"))
m1.2POD
dev.off()
...
png(file = paste0("./Figures/m1.2aPOD_", zval, ".png"))
m1.2aPOD
dev.off()
...
png(file = paste0("./Figures/m1.2bPOD_", zval, ".png"))
m1.2bPOD
dev.off()


save(m1.1POD, m1.2POD, m1.2aPOD, m1.2bPOD, file = paste0("./Figures/H1PODplots_", zval, ".Rdata"))
