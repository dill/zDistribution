...
Although we have determined that primer specificity, technical replication, and number of freeze/thaw cycles affect POD in our samples, we have excluded these from downstream modeling since primers and technical replication were applied consistently across samples.
...
We also used GAMs to address Question 3, and again built iterative models in order to examine covariability between depth (z), latitude, and longitude on a per-species basis. We began by examining whether variability in longitude and latitude (xy) explained a significant amount of remaining variability left in the model after depth variability was account for (Eq. @eq-7), then tested whether x, y, and z covary (i.e. POD \~ xyz). Finally, we allowed the model of xyz covariability to vary independently by species.

$$
\text{logit}(\mu_{i}) = \beta_{0} + f(z_i) + f(x_i,y_i)
$$ {#eq-7}

$$
\text{logit}(\mu_{i}) = \beta_{0} + f(x_i,y_i,z_i)
$$ {#eq-8}
